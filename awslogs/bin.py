...
def main(argv=None):
    ...
    def add_date_range_arguments(parser, default_start='5m'):
        parser.add_argument("-s", "--start", type=str, dest='start', default=default_start, help="Start time (default %(default)s)")
    ...
    get_parser.add_argument("log_group_name", type=str, default="ALL", nargs='?', help="log group name")
    get_parser.add_argument("log_stream_name", type=str, default="ALL", nargs='?', help="log stream name")
    get_parser.add_argument("-f", "--filter-pattern", dest='filter_pattern', help=("A valid CloudWatch Logs filter pattern to "+
        "use for filtering the response. If not provided, all the events are matched."))
    ...
    get_parser.add_argument("-q", "--query", action="store", dest="query", help="JMESPath query to use in filtering the response data")
    ...
    groups_parser.add_argument("-p", "--log-group-prefix", action="store", dest="log_group_prefix", help="List only groups matching the prefix")
